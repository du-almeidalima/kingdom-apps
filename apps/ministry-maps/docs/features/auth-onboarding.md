...
- **Expected UI:** the URL stays `/login` and the `Login` heading remains rendered — neither `LoginPageComponent` nor any app-level listener reacts to the new auth state with a navigation; the header's `#profile-link` (`title="Meu Perfil"`) **does** appear, since it is driven by `UserStateService.isLoggedIn` after the token sign-in resolves the user
...
- **Edge cases:** `consumeInviteLink` is fire-and-forget (`.subscribe()` with no await) — assert the invite mutation with `expect.poll`/`toPass`, not immediately after the landing navigation; if the popup account's email is absent (`providerUser.email` nullish), the mismatch check compares `''` against the invite email and fails → INVALID_EMAIL path (UC-AUTH-18)
- **Priority:** P0 · **Gaps:** **manual-only leg** (OAuth popup); harness extension needed for the seed; the unit-level contract is covered by `firebase-auth-datasource.service.spec.ts`
...
- **Edge cases:** there is no "already signed in" handling on this page at all — a user with a live session sees the same card and can "redeem" the invite for their own account; this is the only path where consumption happens without a corresponding user creation
- **Priority:** P2 · **Gaps:** **manual-only leg** (OAuth popup); harness extension needed for the seed
...
- **Expected UI:** `AuthService`'s constructor subscription to `authStateChanged()` sees a logged-in user disappear (`userState.isLoggedIn && !authStateChange`), clears **both** `UserStateService` and `AuthUserStateService`, and navigates to `/login`; the URL becomes `/login`, the `Login` heading renders, and the header's `#profile-link` disappears (it is gated by `isLoggedIn`)
