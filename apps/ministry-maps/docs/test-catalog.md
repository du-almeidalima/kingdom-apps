...
| UC-ASSIGN-27 | Resume banner after reload/navigation                    | Admin             | P0  | —                               | unit-only (`assign-territories-page.component.spec.ts`)                                  | —                       | `designations_header` + header-linked designations | e2e pending (seed harness ready)             |
| UC-ASSIGN-28 | Multi-submission attaches to the same header             | Admin             | P0  | —                               | unit-only (`designations-header.bo.spec.ts`, `assign-territories.state.service.spec.ts`) | —                       | —                                                  | e2e pending                                  |
| UC-ASSIGN-29 | Manual Stop closes the header (`DONE`/`USER`)            | Admin             | P0  | —                               | unit-only (stop-flow specs)                                                              | —                       | —                                                  | e2e pending                                  |
| UC-ASSIGN-30 | Cart survives navigation, prunes foreign assigns         | Admin             | P1  | —                               | unit-only (`assign-territories.state.service.spec.ts`)                                   | —                       | header w/ foreign designation                      | e2e pending                                  |
