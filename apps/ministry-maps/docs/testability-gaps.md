...
| 7  | ~~Alert resolution truncates `recentHistory`~~ **FIXED 2026-08** | UC-TERR-31, data-model §4.1                              | Resolution now merges the updated subset back into the full `recentHistory` → assert **both** entries survive and only the matched entry changes (`isRevisit: false`)                                    | Only the resolved entries should change ✅          |
...
| 22 | ~~User-edit form always disabled~~ **FIXED 2026-08**      | UC-USERS-05                                              | The gate now keys on the *edited user's* role (`[SUPERINTENDENT, ADMIN, APP_ADMIN].includes`) → non-admin-level targets get a usable form for non-`APP_ADMIN` editors; admin-level targets stay locked                                     | Condition should include `ADMIN`/`SUPERINTENDENT` ✅ |
