...
| 2  | Empty `cities` ternary                                    | UC-TERR-04, UC-ASSIGN-04                                 | `cities.length >= 0 ? cities[0] : ALL` assigns `undefined`; the list/errors collapse → assert no `<option>` selected / list absent                                                        | `length > 0` fallback to `Todas`                  |
