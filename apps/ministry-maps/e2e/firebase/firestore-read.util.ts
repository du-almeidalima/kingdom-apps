 ...
 */
export async function getDoc<T = admin.firestore.DocumentData>(collection: string, id: string): Promise<T | undefined> {
 ...
 */
export function getDocSnapshot(collection: string, id: string): Promise<admin.firestore.DocumentSnapshot> {
  return firestore.collection(collection).doc(id).get();
}

/** Reads all documents of a collection as plain data objects. */
export async function getCollectionDocs<T = admin.firestore.DocumentData>(collection: string): Promise<T[]> {
