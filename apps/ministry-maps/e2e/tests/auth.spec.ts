...
test.describe('Auth', () => {
  ...
  test('UC-AUTH-02 — login renders Google button', async ({ page, db }) => {
    ...
    await expect(loginPage.googleButton).not.toBeDisabled();
  ...
  test('UC-AUTH-08 — Publisher is shown the welcome screen and role is asserted', async ({ page, signInAs, db, seed }) => {
  ...
  const anonymousGuards = ['/home', '/territories', '/territories/assign', '/territories/statistics', '/users', '/welcome'];
  ...
  for (const route of publisherGuards) {
    test(`UC-AUTH-12 — publisher navigating to ${route} redirects to /welcome`, async ({ page, signInAs, db, seed }) => {
      await signInAs('publisher');
      await page.goto(route);
      await expect(page).toHaveURL(/.*\/welcome/);
  ...
  test('UC-AUTH-22 — Admin signout clears state but retains /home (⚠ defect)', async ({ page, signInAs, db }) => {
    ...
    await page.evaluate(async () => {
      await (window as any).__E2E__.auth.signOut();
    });

    await expect(page).toHaveURL(/.*\/home/);
    await expect(page.getByRole('heading', { name: 'Login' })).toHaveCount(0);
    await expect(page.locator('#profile-link')).toHaveCount(0);
    await expect.poll(() => page.evaluate(() => (window as any).__E2E__.auth.currentUser)).toBeNull();
